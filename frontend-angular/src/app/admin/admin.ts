...
export class Admin implements OnInit {
  ...
  guardarHorarioMedico() {
    ...
    // Si el input type="time" da "HH:mm", le añadimos ":00"
    ...
    this.http.put(`${this.apiUrl}/doctors/schedules`, payload,
      { headers: this.headers() }).subscribe({
        next: () => {
          this.horarioGuardado = true;    // muestra el alert de éxito
          this.cargandoHorario = false;
          this.cdr.detectChanges();       // FIX: forzar refresco de la vista
          setTimeout(() => {
            this.horarioGuardado = false;
            this.cdr.detectChanges();
          }, 3000);
